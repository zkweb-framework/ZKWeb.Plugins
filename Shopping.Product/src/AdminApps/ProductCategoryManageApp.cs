...
namespace ZKWeb.Plugins.Shopping.Product.src.AdminApps {
	...
	public class ProductCategoryManageApp : AdminAppBuilder<ProductCategory> {
		...
		public class TableCallback : IAjaxTableCallback<ProductCategory> {
			...
			/// </summary>
			public void OnQuery(
				AjaxTableSearchRequest request, IDatabaseContext context, ref IQueryable<ProductCategory> query) {
				// 按回收站
		...
		public class Form : TabDataEditFormBuilder<ProductCategory, Form> {
			...
			/// 绑定表单
			/// </summary>
			protected override void OnBind(IDatabaseContext context, ProductCategory bindFrom) {
				Name = bindFrom.Name;
			...
			/// 提交表单
			/// </summary>
			protected override object OnSubmit(IDatabaseContext context, ProductCategory saveTo) {
				if (saveTo.Id <= 0) {
