...
namespace ZKWeb.Plugins.Common.PesudoStatic.src.Components.UrlFilters {
	/// <summary>
	/// 伪静态使用的Url过滤器
	/// </summary>
	/// <example>
	...
	public class PesudoStaticUrlFilter : IUrlFilter {
		...
		public void Filter(ref string url) {
			...
			var canIncludeToPesudoPath = new Func<string, bool>(value =>
			...
			if (parameters.Count == 1) {
				...
				if (idParameter != null && idParameter.Count == 1 &&
					canIncludeToPesudoPath(idParameter[0])) {
			...
			foreach (var key in parameters.Keys) {
				...
				if (!canIncludeToPesudoPath(key) ||
					values.Any(value => !canIncludeToPesudoPath(value))) {
