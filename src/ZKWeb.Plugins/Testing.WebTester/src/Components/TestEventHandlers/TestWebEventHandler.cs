...
namespace ZKWeb.Plugins.Testing.WebTester.src.Components.TestEventHandlers {
	/// <summary>
	/// 在网页中运行单元测试使用的事件处理器
	/// </summary>
	public class TestWebEventHandler : ITestEventHandler {
		/// <summary>
		/// 程序集测试开始
		/// </summary>
		public void OnAllTestStarting(AllTestStartingInfo info) {
			...
			webTesterManager.UpdateInformation(assemblyName, testInfo => {
				testInfo.State = AssemblyTestState.Running;
				testInfo.Updated();
			});
		}
		...
		public void OnAllTestCompleted(AllTestCompletedInfo info) {
			...
			var webTesterManager = Application.Ioc.Resolve<WebTesterManager>();
			webTesterManager.UpdateInformation(assemblyName, testInfo => {
				testInfo.State = AssemblyTestState.FinishedRunning;
